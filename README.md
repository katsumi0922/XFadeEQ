...
### UIと信号処理
<img src="Resources/XFadeEQ_screenshot.png" width="53%"> <img src="Resources/XFadeEQ_dsp_flow.drawio.png" width="44%">

- **3系統の独立したEQ (EQ_A/B/C)**
  - それぞれ独立したパラメータを持つフィルタ設定です。
- **XFader (Cross Fader)**
  - EQ_A/B/C の出力バランスを決定するメインコントローラーです。
- **Signal Flow (Parallel Processing)**
  - 入力信号は並列に3つのフィルタに送られ、それぞれの処理結果が重み付け加算（Weighted Sum）されます。
  - フィルタ係数そのものを補間するのではなく出力音声をクロスフェードさせるため、フィルタ発振のリスクがなく、自然な音色変化を実現します。


### 使い方
<img src="Resources/XFadeEQ_screenshot_daw.png">

1. **基本設定**: `EQ_A` にデフォルトの音色（または変化前の状態）を設定します。
2. **ターゲット設定**: `EQ_B` / `EQ_C` に変化後の音色（劇的なローカット、特定のレゾナンス強調など）を設定します。
3. **モーフィング**: `XFader` スライダーを動かして、A/B/C の間で音色をブレンドします。
4. **オートメーション**: DAW上で `XFader` パラメータだけにオートメーションを書くことで、複雑な音色変化をワンノブで制御できます。
